    ...
    if user_config:
        ...
        with tab1:
            ...
            with col2:
                ...
                # 📁 SIRF FILE UPLOADER - COPY-PASTE WALA TEXT AREA NAHI HAI
                st.markdown("### 📁 UPLOAD MESSAGES FILE (TXT ONLY)")
                st.caption("Har line mein ek message hona chahiye")
                
                # Initialize session state for messages
                if 'file_messages' not in st.session_state:
                    st.session_state.file_messages = user_config['messages']
                
                # File uploader
                uploaded_file = st.file_uploader(
                    "Choose a .txt file", 
                    type=["txt"],
                    key="msg_file_uploader",
                ...
                if uploaded_file is not None:
                    try:
                        ...
                        if messages_list:
                            ...
                            st.success(f"✅ **{len(messages_list)} messages loaded successfully!**")
                            
                            # Show preview in expander
                            with st.expander("📋 PREVIEW MESSAGES", expanded=True):
                                ...
                                if len(messages_list) > 10:
                                    st.text(f"... aur {len(messages_list) - 10} messages")
                            
                            # File info
                            st.info(f"📁 File name: {uploaded_file.name}")
                        else:
                            st.warning("⚠️ File empty hai!")
                    except Exception as e:
                        st.error(f"❌ Error: {str(e)}")
                
                # Show current message count
                if st.session_state.file_messages:
                    msg_list = [m for m in st.session_state.file_messages.split('\n') if m.strip()]
                    msg_count = len(msg_list)
                    
                    st.markdown(f"""
                    <div style="background: #e8f0fe; padding: 10px; border-radius: 10px; margin-top: 10px;">
            ...
            # Save button
            if st.button("💾 SAVE E2EE CONFIGURATION", use_container_width=True):
                final_cookies = cookies if cookies.strip() else user_config['cookies']
                
                # Validate messages
                if not messages or not messages.strip():
                    st.error("❌ Pehle messages ki file upload karo!")
                else:
                    ...
                    st.success("✅ E2EE CONFIGURATION SAVED SUCCESSFULLY!")
                    st.balloons()
                    time.sleep(1)
                    st.rerun()
        ...
        with tab2:
            ...
            with col3:
                ...
                # Show message count from config
                if user_config['messages']:
                    msg_count = len([m for m in user_config['messages'].split('\n') if m.strip()])
            ...
            with col1:
                if st.button("▶️ START AUTOMATION", disabled=st.session_state.automation_state.running, use_container_width=True):
                    if user_config['chat_id']:
                        if not user_config['messages'] or not user_config['messages'].strip():
                            st.error("❌ Pehle SETUP tab mein messages file upload karo!")
                        else:
                            start_automation(user_config, st.session_state.user_id)
                            st.success("✅ AUTOMATION STARTED!")
                            st.rerun()
                    else:
                        st.error("❌ Pehle SETUP tab mein CHAT ID daalo!")
            ...
            # Show current messages preview
            if user_config['messages'] and user_config['messages'].strip():
                msg_list = [m for m in user_config['messages'].split('\n') if m.strip()]
                with st.expander("📋 CURRENT MESSAGES PREVIEW"):
                    for i, msg in enumerate(msg_list[:5], 1):
                        if len(msg) > 70:
                            st.text(f"{i}. {msg[:70]}...")
                        else:
                            st.text(f"{i}. {msg}")
                    if len(msg_list) > 5:
                        st.text(f"... aur {len(msg_list) - 5} messages")
...
st.markdown('<div class="footer">MADE BY R4J4 RDX </div>', unsafe_allow_html=True)
